...
class GameEngine:
    ...
    def process_input(self, input_text: str) -> str:
        ...
        if parts[0] == "look" and len(parts) > 1 and parts[1] == "around":
            ...
            npc = parts[1]
            return self._handle_talk_command(npc)
        
        elif parts[0] in ["go", "walk", "move", "head"] and len(parts) > 1:
            direction = parts[1]
            return self._handle_movement(direction)
        
        elif parts[0] in ["exits", "directions", "connections"] or (len(parts) >= 3 and parts[0] == "where" and parts[1] == "can" and parts[2] == "i" and parts[3] == "go"):
        ...
        elif parts[0] == "quicksave" or (len(parts) == 2 and parts[0] == "quick" and parts[1] == "save"):
        ...
        elif parts[0] == "quickload" or (len(parts) == 2 and parts[0] == "quick" and parts[1] == "load"):
            return self._handle_quick_load_command()
        
        elif parts[0] == "saves" or (len(parts) == 2 and parts[0] == "list" and parts[1] == "saves"):
            return self._handle_list_saves_command()
        
        elif parts[0] in ["quests", "journal"] or (len(parts) == 2 and parts[0] == "quest" and parts[1] == "log"):
    ...
    def _handle_look_around(self) -> str:
        ...
        npcs_here = [npc for npc in self.config.npcs.values() 
    ...
    def _handle_talk_command(self, npc_name: str) -> str:
        """Handle the 'talk to' command."""
        # Find an NPC in the current location by name (partial match)
        matching_npcs = []
        for npc in self.config.npcs.values():
            # Check if NPC is in current location and name matches
            if npc.location == self.current_location and npc_name.lower() in npc.name.lower():
                matching_npcs.append(npc)
            # Check pronoun matching if no direct name match
            elif npc.location == self.current_location and self._matches_pronoun(npc, npc_name):
                matching_npcs.append(npc)
        ...
        if len(matching_npcs) == 1:
            ...
            self.game_state.record_npc_interaction(npc.id)
            
            # This would be replaced with actual dialogue handling
            return f"You start a conversation with {npc.name}."
        
        # Multiple matches
        npc_list = ", ".join(npc.name for npc in matching_npcs)
        return f"Did you mean one of these people: {npc_list}?"
    ...
    def _handle_movement(self, direction: str) -> str:
        ...
        # Try to match direction with a connected location
            ...
            for loc_id in current_location.connected_locations:
                ...
                if (direction.lower() in connected_loc.name.lower() or 
                    direction.lower() in loc_id.lower()):
                    target_location_id = loc_id
                    break
                
                # Match by cardinal direction
                if direction in ["north", "south", "east", "west"]:
                    if (direction in connected_loc.name.lower() or 
                        direction in loc_id.lower()):
                        target_location_id = loc_id
                        break
    ...
    def _handle_quest_info(self, quest_id: str) -> str:
        ...
        current_stage_desc = current_stage.description if current_stage else "No active stage"
        
        # Calculate progress percentage
        completed_stages = sum(1 for stage in quest.stages if stage.status == "Completed")
    ...
    def _matches_pronoun(self, npc, pronoun: str) -> bool:
        """Check if NPC matches a pronoun."""
        pronoun = pronoun.lower()
        
        # Count NPCs of each gender in current location to avoid ambiguity
        males_in_location = sum(1 for n in self.config.npcs.values() 
                               if n.location == self.current_location and n.gender == "male")
        females_in_location = sum(1 for n in self.config.npcs.values() 
                                 if n.location == self.current_location and n.gender == "female")
        
        if pronoun in ["him", "he", "his"] and npc.gender == "male" and males_in_location == 1:
            return True
        
        if pronoun in ["her", "she", "hers"] and npc.gender == "female" and females_in_location == 1:
            return True
